...
def process_tourism_data(excel_file, year, starting_month_index, ending_month_index):
    ...
    for month in available_months:
        ...
        total_from_row = float(df.iloc[-1][month]) if pd.notna(df.iloc[-1][month]) else calculated_total
    ...
    for _, row in df.iterrows():
        ...
        total_from_column = float(row['Total']) if 'Total' in df.columns and pd.notna(row['Total']) else 0

        # Calculate total by summing available monthly values
        calculated_total = float(sum(row[month] for month in available_months if pd.notna(row[month])))
...
def save_to_mongodb(monthly_data, country_data, year):
    try:
        ...
        print("Monthly data sample:", monthly_data[0] if monthly_data else "No monthly data")
        print("Country data sample:", country_data[0] if country_data else "No country data")
        return False
...
def main():
    ...
    for excel_file in tourism_dir.glob('*.xlsx'):
        ...
        try:
            ...
            # Process the data
            monthly_data, country_data = process_tourism_data(excel_file, year, starting_month_index, ending_month_index)
